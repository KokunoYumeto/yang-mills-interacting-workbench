...
\eqref{eq:full-damped-state} at fixed $L,a$. No unbounded first moment is
...
\eqref{eq:full-finite-measure} unchanged.

\begin{theorem}\label{thm:full-comparison-limit}
For any $a\downarrow0$, $\ell=a(2L+1)\longrightarrow\infty$, any
real $h\in C_c^\infty(\mathbb R^3)$, and every $t>0$, the complete
sum in \eqref{eq:full-finite-measure} converges to
...
\eqref{eq:full-pair-native}; this gives such a $K_h$ directly.
...
$\ell\ge1$. It follows from \eqref{eq:full-uniform-pair-bound} that
...
location of its component, \eqref{eq:full-mode-cochain} is exactly a
product of
...
terms in \eqref{eq:full-pair-native}, with uniform error
...
\eqref{eq:full-parity-unitary}. Its unitarity replaces the two
...
Multiplication by $3/8$ proves \eqref{eq:full-momentum-integral}.
...
\begin{theorem}\label{thm:full-density}
The locally finite raw measure associated to
\eqref{eq:full-momentum-integral} is absolutely continuous, with
\begin{align}
...
\end{theorem}
\begin{proof}
In \eqref{eq:full-momentum-integral} set $k=p+q$, $r=|p|$,
...
\eqref{eq:full-density}. Integrating $e^{-t\omega}$ from $K$
to infinity proves \eqref{eq:full-laplace}.
...
The physical subspace used here is invariant under simultaneous
adjoint $\SU(2)$ rotations of the three colours. Orthogonality
...
\ref{thm:full-comparison-limit} therefore gives convergence for
...
function \eqref{eq:full-laplace} is continuous on $(0,\infty)$
...
\eqref{eq:full-state-map}. Indeed the mixed time argument is
...
\eqref{eq:full-small-energy} and the substitution $v=t\omega$
give
...
\eqref{eq:full-actual-norm-energy} transfers both raw quantities at
...
defines the kernel \eqref{eq:full-state-map} by its bounded Fourier
...
where all $A,\beta$ are retained. Direct change of variables gives
\begin{align}
...
\eqref{eq:full-large-time} applies with its exact squared
coefficient $\lambda^4\|\nabla\times u(s)\|_2^4$, and
\eqref{eq:full-L1-bound} bounds its raw norm with the same
