...
\eqref{eq:low-pair}; here the face matrix $M_f$ is not the Hilbert
frame $M_g$. Set
\[
...
\eqref{eq:local-normal-order}. No magnetic or number-preserving term
is discarded.
...
The graph result \eqref{eq:local-graph} proves
...
\begin{theorem}\label{thm:local-coefficients}
...
\end{theorem}
\begin{proof}
Let $V_i$ be the original edge functions \eqref{eq:low-modes} and
...
integral: \eqref{eq:local-quadrupole} is an explicit map to three
...
Theorem~\ref{thm:low-actual-sequence}, $\|G_{g_j}-I\|<\epsilon_j$,
and
\[
...
\begin{theorem}\label{thm:local-common}
...
\end{theorem}
\begin{proof}
Apply \eqref{eq:local-leading} to each term of
\eqref{eq:local-compression}, and use
...
$4\sqrt2\pi H_hB$. This proves \eqref{eq:local-T}, including its
...
and $\ell_j\Delta_j\to2\sqrt2\pi$ give \eqref{eq:local-time}.
...
of $\Xi$ vanishes. Equations \eqref{eq:local-frame}--\eqref{eq:local-T}
...
\eqref{eq:local-T}. For the sign and constant in the second,
...
are \eqref{eq:local-mass}, \eqref{eq:local-fixed-time} and
\eqref{eq:local-slow-time}, with
\[
...
symmetric mode matrix. From \eqref{eq:local-normal-order},
\begin{align}
...
The two terms on the right in \eqref{eq:local-leak-pair} have
...
covariance in \eqref{eq:local-leakage} at fixed box and coupling limit.
