...
Put $d=3r$ and keep the exact chart (\ref{eq:low-chart}), with
...
\eqref{eq:vertex-exact-field} retains exactly the $t_{ce}$ term.
...
degree three using \eqref{eq:vertex-pauli}, and then use
...
\eqref{eq:vertex-full-H} and $H_{\rm osc}+gH^{(1)}+g^2H^{(2)}$;
...
\eqref{eq:vertex-K}, positivity of $K$, and
...
covariance is exactly $2K^{-1}$ by \eqref{eq:vertex-K}.
...
\begin{theorem}\label{thm:vertex-nonzero}
...
Equation~\eqref{eq:vertex-W234} gives the magnetic coefficient
$-1/(8a)$.
...
in \eqref{eq:vertex-cubic-polynomial}; all original tree paths in
$b$ are retained.
...
\end{equation}
The two matrices in \eqref{eq:vertex-certificate-matrices} are
...
\eqref{eq:vertex-certificate-matrices} into
\eqref{eq:vertex-certificate-contraction}, and adding the exact
...
\eqref{eq:vertex-raw-moments} is at least
$2\sum_I|c_I|^2/\sqrt3$, which with
\eqref{eq:vertex-strict-coefficient} gives the first bound in
\eqref{eq:vertex-lower-bounds}. Each $E_I\le6\sqrt3/a$, so
...
by its norm. Substitution of \eqref{eq:vertex-quasimode-coefficients}
into the exact operator, using \eqref{eq:vertex-remainder-bound}, gives
...
\eqref{eq:vertex-quasimode-residual} have been left unchanged.
In particular the second lower bound of
\eqref{eq:vertex-lower-bounds} proves a nonzero first derivative
...
map. Equation~\eqref{eq:vertex-actual-vacuum} gives
...
the norm error in \eqref{eq:vertex-actual-vacuum} controls every
...
coordinate derivative in \eqref{eq:vertex-response-connection}.
...
\eqref{eq:vertex-full-H}--\eqref{eq:vertex-H12}, with each electric
...
\eqref{eq:vertex-Wk}; no fifth or sixth power in an oriented factor
...
\eqref{eq:vertex-remainder-bound}, proves that
...
\eqref{eq:vertex-weighted-graph-bound}. The term along $\Phi_0$
...
simplex in \eqref{eq:vertex-semigroup-coefficients}; the repeated
cases follow by direct integration of the resulting linear factor.
Only finitely many occupations enter \eqref{eq:vertex-C2}, because
...
Thus both sides of \eqref{eq:vertex-C2} vanish, as the exact
