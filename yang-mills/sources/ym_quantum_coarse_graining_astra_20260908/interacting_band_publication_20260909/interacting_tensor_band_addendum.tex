...
To avoid confusing the Gaussian matrix $K$ of (\ref{eq:vertex-K})
...
$(x+y)^2\le2(x^2+y^2)$ and (\ref{eq:band-exact-graph}) prove
...
bound; (\ref{eq:band-weighted-graph}) proves why that loss is unnecessary
...
(\ref{eq:vertex-cubic-polynomial}), by the cyclic determinant identity.
...
of (\ref{eq:vertex-H12}), then the displayed Hermite polynomial and
...
(\ref{eq:band-full-K}) are at least $\sigma/a>0$. Every reached
...
(\ref{eq:band-full-K}); expand its finite Hermite polynomial and
...
The perpendicular choice in (\ref{eq:band-zeta}) therefore gives
\begin{align}
...
$q(S)$, with $R=\mathsf R_S^{\min}$ from (\ref{eq:low-block}), is
...
in (\ref{eq:band-q}) and recover the unchanged mass
$d_{L,a}(S)$ of (\ref{eq:low-d}).
...
(\ref{eq:band-weighted-graph}), and the column expansion above give
...
their ordinary Euclidean inner product. Multiplying (\ref{eq:band-exact-inverse})
...
At comparison coupling, (\ref{eq:band-F-inverse}) gives the same map
...
The exact norm is $q_g^*\mathcal G_gq_g$, so (\ref{eq:band-raw-Gram-energy})
proves
\begin{align}
...
in both quantities in (\ref{eq:band-raw-expansions}); they were not
...
In fact the exact inverse (\ref{eq:band-exact-inverse}) represents
...
gap, with the complete coefficient (\ref{eq:band-full-K}).
...
(\ref{eq:band-raw-Gram-energy}). The matrix exponential converges
...
(\ref{eq:band-long-time}). Both states are exactly in the actual
...
the semigroup parameter in (\ref{eq:band-long-time}). At fixed
$L,a,g$, linearity of (\ref{eq:band-tensor-map}) proves
\begin{align}
...
(\ref{eq:band-tensor-Rayleigh}). This is the exact way the given
...
the entire structure of (\ref{eq:band-full-K}). We give the exact
...
(\ref{eq:low-modes}). In the six coordinates, this sends the
...
(\ref{eq:band-full-K}), each with original units $a^{-1}$.
...
are $c_oS_{ij}$. This proves (\ref{eq:band-channel-masses}) and
$N_s+N_t+N_o=d_{L,a}(S)$, retaining the three positive raw masses.
Consequently the full coefficient in (\ref{eq:band-tensor-Rayleigh}) is
\begin{equation}\label{eq:band-channel-correction}
 \frac{k_sN_s(S)+k_tN_t(S)+k_oN_o(S)}{N_s(S)+N_t(S)+N_o(S)}.
\end{equation}
Similarly the limit in (\ref{eq:band-long-time}) for equal tensors
...
(\ref{eq:band-channel-correction}), not a sign assigned from one
...
real, the inverse (\ref{eq:band-exact-inverse}) gives a real tensor
...
At $s=s_0$, (\ref{eq:band-fluid-sampling}) gives the prescribed
...
using (\ref{eq:band-h-tensor}); the right inverse proves that the
