...
## 8. Exact Hamiltonian filtering, including the zero-atom distinction

For any finite regulator and smooth vacuum-orthogonal \(v\), put
\(A=H-\mathcal E\) and
\[
 u_t=e^{-tA/2}v,\quad
 C(t)=\|u_t\|^2=\int_{[0,\infty)}e^{-t\omega}\,d\nu_v(\omega).
\tag{31}
\]
This is the raw spectral measure, with no normalization by a possibly
vanishing norm. Since the vectors used above lie in the operator domain,
spectral differentiation for \(t>0\) gives
\[
 -C'(t)=\int\omega e^{-t\omega}d\nu_v(\omega),\quad
 m(t):=-C'(t)/C(t),\quad
 m'(t)=-\operatorname{Var}_{\nu_{v,t}}(\omega)\le0,
\tag{32}
\]
where \(d\nu_{v,t}=e^{-t\omega}d\nu_v/C(t)\). Euclidean-time filtering
therefore lowers the exact spectral mean while retaining the raw amplitude
\(C(t)\).

For a varying regulator, Markov's inequality gives the exact criterion
\[
 \nu_{v_j}([0,R])\ge C_j(t_j)\left(1-\frac{m_j(t_j)}R\right),
 \qquad R>0.\tag{33}
\]
Equation (33) is a sufficient estimate on raw low-energy mass, not a
necessary criterion for gaplessness. A previous version incorrectly called
a positive lower bound on \(C_j(t_j)\) necessary. The following exact
calculation corrects that statement.

Take \(k=3/2\), \(c=100\sqrt2\pi\), and the finite measure
\[
 d\gamma_c(\omega)=
 \frac{\omega^{k-1}e^{-\omega/c}}{\Gamma(k)c^k}\,d\omega.
\]
On \(\mathbb C\Omega\oplus L^2(\gamma_c)\), let
\(K=0\oplus M_\omega\), where multiplication by \(\omega\) has domain
\(\{f:\int\omega^2|f|^2d\gamma_c<\infty\}\). This real multiplication
operator is self-adjoint: its resolvent at nonreal \(z\) is multiplication
by \((\omega-z)^{-1}\), and its adjoint has the same maximal domain.
Absolute continuity gives \(\ker K=\mathbb C\Omega\). For each
\(\varepsilon>0\), \((0,\mathbf1_{(0,\varepsilon)})\) is a nonzero
vacuum-orthogonal vector of energy quotient at most \(\varepsilon\).
Consequently this operator has no positive gap. For its fixed raw vector
\(v=(0,1)\), direct Gamma integration gives
\[
 C(t)=(1+ct)^{-k},\qquad
 m(t)=\frac{kc}{1+ct},\qquad
 m'(t)=-\frac{kc^2}{(1+ct)^2}.
\]
Thus both \(C(t)\to0\) and \(m(t)\to0\). No positive lower bound on the
filtered norm is necessary.

In fact, suppose finite raw measures \(\nu_j\) converge weakly to a finite
\(\nu\), \(\liminf C_j(t_j)\ge\eta>0\), and \(m_j(t_j)\to0\).
For every \(R>0\), (33) gives
\(\liminf\nu_j([0,R])\ge\eta\). Since \([0,R]\) is closed, the
portmanteau inequality then gives \(\nu([0,R])\ge\eta\).
Continuity from above as \(R\downarrow0\) proves \(\nu(\{0\})\ge\eta\).
Under finite weak convergence the proposed pair of bounds therefore forces
a positive zero atom, rather than detecting only positive energies
arbitrarily close to zero.

For a fixed positive Hamiltonian on a fixed Hilbert space, a family of
nonzero vectors orthogonal to its entire kernel with Rayleigh quotients
tending to zero does contradict a positive spectral gap, regardless of the
raw norms: a gap \(\Delta>0\) would imply
\(\langle v,Kv\rangle\ge\Delta\|v\|^2\) for every such vector.
For varying regulators, the additional issue is to construct the actual
common state and observable limit identifying the continuum theory.
It is not a defect in that variational inequality.

The fixed-parameter radial family in Section 7 of the companion
*Actual weak-coupling vacuum, observable and spectral maps* has
\(C_j(t)\to1-2^{-3/2}\) and \(m_j(t)\to0\) at fixed physical \(t>0\),
so it retains an extra zero sector. Differentiation of its limit at such
positive times is justified because \(\omega e^{-t\omega}\) is bounded.
The native electric family here has the vanishing finite-energy fraction
(29)--(30). The growing-parameter radial family in the separate Gamma
note instead has the atomless measure displayed above. These are three
explicit spectral outcomes of specified states, not a classification of
all possible state maps.
