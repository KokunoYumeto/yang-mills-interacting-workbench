...
These statements prove \eqref{eq:low-mag} globally, for signed weights
...
\begin{theorem}\label{thm:low-block}
On the three-mode basis \eqref{eq:low-modes}, the exact block
$\mathsf R_S^{\min}$ of \eqref{eq:low-pair} is
...
\]
Substitution gives the first equality in \eqref{eq:low-moments}.
...
The common frequency factors in \eqref{eq:low-pair} give the diagonal
part of \eqref{eq:low-block}.
...
face weights. The negative electric sign in \eqref{eq:low-pair}
gives the positive off-diagonal entry in \eqref{eq:low-block}.
...
squared norms in \eqref{eq:low-full-measure} give \eqref{eq:low-d}.
\end{proof}
...
For $S\ne0$, \eqref{eq:low-vector} and spectral projection convergence
prove
...
including all higher mode pairs in \eqref{eq:low-full-measure}.
...
tends to \eqref{eq:low-fraction}.
...
Equation \eqref{eq:low-pair} becomes
\[
...
\eqref{eq:low-fraction-bound}. The upper bound follows because
...
\begin{theorem}\label{thm:low-actual-sequence}
...
\eqref{eq:low-positive-g} imply that every sufficiently small $g>0$
...
The projection in \eqref{eq:low-actual-vector} has rank six.
...
\]
Insert these into the full expression \eqref{eq:low-d}, using
...
inverse in Theorem~\ref{thm:low-block}.
...
Section~\ref{sec:local-band} constructs the next common
