...
For clarity, in the original link representation their displayed
maximal-tree Hamiltonian corresponds to
\(H_{\rm B}(g_{\rm B},a)=g_{\rm B}^2\sum_eE_e/(2a)
+\sum_p(2-\operatorname{tr}U_p)/(g_{\rm B}^2a)\).
The exact parameter/energy dictionary for the same edge Casimir and
face trace is
\[
 H_g=\sqrt2\,H_{\rm B}(2^{3/4}g,a).
\]
Substitution gives respectively the original coefficients \(2g^2/a\)
and \(1/(2g^2a)\) in (1). This is an explicit operator identity on
the common original-link domain; its energy multiplier is not dropped.
Each chosen maximal tree is then related to that link representation by
its own exact Haar unitary (5).
